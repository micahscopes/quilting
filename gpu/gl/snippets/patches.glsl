...
Patch bilinearQuad(
  ...
  float u, float v) {
    ...
    float uu = u+eps;
    CGA3 W0_uu = mul(abs(1.0-uu)*(1.0-v),  weight0); // 0, 0
    CGA3 W1_uu = mul(abs(uu)*(1.0-v),        weight1); // 1, 0
    CGA3 W2_uu = mul(abs(1.0-uu)*v,        weight2); // 0, 1
    CGA3 W3_uu = mul(abs(uu)*v,              weight3); // 1, 1
    CGA3 top_uu = add(
      mul(point0, W0_uu),
      mul(point1, W1_uu),
      mul(point2, W2_uu),
    ...
    float vv = v+eps;
    CGA3 W0_vv = mul((1.0-u)*abs(1.0-vv),  weight0); // 0, 0
    CGA3 W1_vv = mul(u*abs(1.0-vv),        weight1); // 1, 0
    CGA3 W2_vv = mul((1.0-u)*abs(vv),        weight2); // 0, 1
    CGA3 W3_vv = mul(u*abs(vv),              weight3); // 1, 1
    ...
    // vec3 normal = abs(normalize(vec3(Normal.e12, Normal.e23, Normal.e13)));
    vec3 x_uu = normalize(vec3(X_uu.e1, X_uu.e2, X_uu.e3)-x);
    vec3 x_vv = normalize(vec3(X_vv.e1, X_vv.e2, X_vv.e3)-x);
    return Patch(x, abs(normalize(cross(x_uu, x_vv))));
}