...
function incircle(a,b,c,d) {
  const [u,v,w]=[a,b,c].map(p=>{const x=BigInt(p[0]-d[0]),y=BigInt(p[1]-d[1]);return [x,y,x*x+y*y];});
...
export function replayRepairOrder(snapshot,{priority=(slot)=>slot,budget=2048}={}) {
  ...
  for(let round=0;round<budget;round++) {
    ...
    for(let slot=0;slot<twins.length;slot++) {
      ...
      const circle=incircle(points[u],points[v],points[a],points[b]);
