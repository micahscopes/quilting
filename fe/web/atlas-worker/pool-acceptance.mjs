...
import {auditDelaunay} from './packed-audit.mjs';
  ...
  const scopes=original.map((item,index)=>({...item,scope:{
    ...
    async request(lane,payload,signal) {
      ...
      try {
        ...
        const delaunay=auditDelaunay(response.tile,payload.key.square,payload.key);
        receipts.push({worker:index,ordinal:payload.lease.ordinal,key:payload.key,
          ...
          bytes:response.tile.words.byteLength,roundtripMs,auditMs:performance.now()-auditStart,delaunay});
        return response;
      } finally {active--;}
    },
  }}));
