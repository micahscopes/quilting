...
fn fs_pbr(@builtin(front_facing) front_facing: bool, in: FragInput) -> PbrOutput {
    ...
    if pbr.unlit > 0.5 {
        ...
        return PbrOutput(vec4<f32>(unlit_color, alpha), vec4<f32>(in.mobius_stretch, in.frag_coord.z, 0.0, 1.0));
    }
    ...
    if pbr.debug_output > 0.5 {
        ...
        if !front_facing {
            return PbrOutput(vec4<f32>(dbg.r * 0.3 + 0.7, dbg.g * 0.3, dbg.b * 0.3, in.fade), vec4<f32>(in.mobius_stretch, in.frag_coord.z, 0.0, 1.0));
        }
        return PbrOutput(vec4<f32>(dbg, in.fade), vec4<f32>(in.mobius_stretch, in.frag_coord.z, 0.0, 1.0));
    }
    ...
    return PbrOutput(vec4<f32>(color, alpha * in.fade), vec4<f32>(in.mobius_stretch, in.frag_coord.z, 0.0, 1.0));
}
