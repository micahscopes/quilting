...
use hyperscape::{
    ...
    FocusSphere, InteractionAction, InteractionHit, InteractionPickAuthority,
    InteractionPickAuthorityDisposition, InteractionPickEvidenceObserver, InteractionTarget,
    InteractionTargetSample, InteractionTargetTable, LayerTransform, MappedSpaceMouseFrame,
    NavigationAction, NavigationFrame, NavigationPreset,
...
impl HyperscopeAppShadow {
    ...
    pub async fn pick_backend_surface(&self, x: i32, y: i32) -> Result<JsValue, JsValue> {
        let pixel = [
        ...
        let result = |disposition, hit, surface| ShadowBackendPickAuthorityResult {
        ...
        let readback = match capture.read().await {
            ...
            Err(error) => {
                ...
                return to_js(&result(disposition, None, None));
            }
        };
        ...
        let resolved = match readback.hit {
            ...
            Some(raw) => (|| {
                ...
                Ok::<_, String>((
                    Some(ShadowInteractionHit::from(hit)),
                    Some(surface),
                ))
            })(),
        };
        ...
        let disposition = self
            ...
            .borrow_mut()
            .accept(request);
        to_js(&result(disposition, hit, surface))
    }
