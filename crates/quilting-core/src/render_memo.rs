//! Device-epoch memoization for lowered renderer resources.
//!
//! Keys are immutable descriptors from [`crate::render_pipeline`]; values are
//! concrete WebGL/WebGPU effects. The cache never owns invalidation policy:
//! changing a context/device epoch returns every old value so the backend can
//! explicitly release it with the API that created it.
    ...
    K: Eq + Hash,
{
    ...
    /// is counted but never inserted, so partial construction cannot poison a
    /// later attempt. The creation closure remains responsible for releasing
    /// raw objects it made before returning `Err`.
    ...
    /// Begin a new backend generation and return old resources to the caller.
    /// Repeating the same epoch is a no-op and preserves every entry.
...
mod tests {
    ...
    #[test]
    fn equal_keys_lower_once_and_failed_effects_are_not_retained() {
    ...
    #[test]
    fn context_epoch_returns_resources_for_explicit_release() {
