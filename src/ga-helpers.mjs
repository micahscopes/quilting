...
export const Mul4 = (w, x, y, z) => Alg.Mul(Alg.Mul(w, x), Alg.Mul(y, z));
...
export const up = (x) =>
  reduce(
    [no, x, Alg.Mul(Mul3(x, x, ni), Alg.Scalar(0.5))],
    (x, y) => Alg.Add(x, y),
    Alg.Scalar(0)
  );

export const pointWeight = (point, weight) =>
  Alg.Add(
    Alg.Mul4(point, ni, weight, Alg.Scalar(0.5)),
    weight
  );
